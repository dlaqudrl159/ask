...
# 객체지향 프로그래밍이란?
  ...
  - 이런 형변환이 가능함으로써 새로운 자식 클래스(예를 들면 유럽) 같은 걸 추가해도 Earth를 상속받기만 하면 되니 확장성이 용이하며 showTime() 이라는 공통메서드를 활용해 코드의 재사용성이 올라간다.
   ...
   ## 상속
  - 상속이란 부모클래스의 필드와 메소드를 물려받아 자식 클래스에서 사용하는 것
 ...
 ## 상속
 - 부모클래스의 속성과 기능들을 물려받아 자식클래스에서 재사용이 가능한 것입니다. 코드의 재사용성이 증가하고 중복을 제거할 수 있습니다.
   추상클래스에 추상메서드를 사용해서 공통의 기능을 강제할 수 있고 인터페이스를 사용해서 정해진 규격을 사용하게 할 수 있습니다. ide를 사용할때 인터페이스를 상속받으면 추상메서드를 강요하는데요 정해진 규격을 강제한다는 뜻 입니다.
## 객체
   - 프로그래밍에서 객체란 프로그램의 단위로써 어떤 사물의 데이터나 속성 기능들을 묶어 놓은것입니다.
## 인스턴스
   - new 연산자를 통해 만들어진 객체가 메모리상에 올라가있으면 인스턴스라고 부릅니다 예를 들어 자동차라는 객체를 만들때 빨간색 속성을 가진 A자동차 가 있고 파란색 속성을 가진 B자동차 이 2개의 자동차를 인스턴스라고 부릅니다.
## 변수
   - 변수란 하나의 값을 저장할 수 있는 메모리 공간 입니다.
## 상수
   - 상수란 변하지 않는 변수 입니다. final이라는 키워드를 붙혀 정의합니다.
## 메서드
   - 메서드는 객체의 기능을 정의해 놓은것입니다.
## static이란
   - static이란 공유의 뜻을 가지고 있습니다. static이 붙으면 객체생성없이 사용가능하며 
# 톰캣
